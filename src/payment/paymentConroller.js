...
//   try {
//     const { courseId, currency, paymentMethod, customer } = req.body;
...
//     // Step 1: Authentication with Paymob API
//     const authResponse = await axios.post('https://accept.paymob.com/api/auth/tokens', {
//       api_key: PAYMOB_API_KEY,
//     });
...
// // Webhook to handle payment status updates from Paymob
// exports.paymentWebhook = async (req, res) => {
//   try {
//     const data = req.body;
//     const receivedHmac = req.query.hmac;
...
//       return res.status(400).send('Payment not found or already refunded');
//     }

//     // Initiate refund through the payment gateway (e.g., Paymob)
//     const refundResponse = await axios.post('https://accept.paymob.com/api/ecommerce/refund', {
...
// // Utility function to calculate HMAC
// const calculateHmac = (data, secret) => {
//   const message = `${data.amount_cents}${data.created_at}${data.currency}${data.order.id}${data.success}`;
//   const hash = crypto.createHmac('sha512', secret).update(message).digest('hex');
//   return hash;
// };
